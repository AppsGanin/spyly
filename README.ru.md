...
**Записывает разговоры, расшифровывает их по участникам и отдаёт готовый текст кодинг-агенту.**
...
- **Пишет вас и собеседников раздельно.** Микрофон и системный звук — две независимые дорожки, поэтому видно, кто что сказал, и эхо не удваивает фразы.
- **Разделяет по голосам обе дорожки.** Несколько человек вокруг одного микрофона в переговорке тоже разделятся.
- **Показывает текст по ходу разговора.** Слова появляются, пока вы говорите: замер на Mac с процессором Apple — 0.5–0.8 секунды.
- **Узнаёт постоянных участников.** Назвали человека один раз — со следующей записи имя подставится само по слепку голоса.
...
| `get_transcript`, `get_summary` | сам разговор — целиком, по участнику или по куску времени |
...
```
микрофон ────┐
             ├─→ две дорожки WAV ─→ расшифровка ─→ голоса ─→ участники ─→ конспект
системный ───┘        │
...
~/Spyly/meetings/2026-08-27--billing-worker--a1b2/
  ...
  transcript.json    участники, реплики, времена слов
...
| GigaAM v3 | лучший русский, сама расставляет знаки препинания | 500 МБ |
| Parakeet TDT v3 | заметно быстрее Whisper, 25 языков | 638 МБ |
| Nemotron Speech 3.5 | потоковая — на ней работает живой текст | 475 МБ |

Разделение по голосам — pyannote segmentation 3.0 (7 МБ) и слепки 3D-Speaker (40 МБ).
...
| `⌘Z` / `⌘⇧Z` | отменить или вернуть правку записи |
| `⌘F` | поиск |
| `⌘,` | настройки |

## Приватность

Расшифровка и разделение по голосам по умолчанию идут локально. Слепки голосов — биометрия: лежат только на этом компьютере, ни в один облачный провайдер не отправляются, удаляются из настроек. Календарь читается только на чтение и только вокруг текущего момента.
...
Чужие части сохраняют свои лицензии: whisper.cpp под MIT, sherpa-onnx под Apache-2.0, шрифты Geist под SIL Open Font License. Модели распознавания скачиваются при работе и живут по своим условиям — у GigaAM, например, коммерческое использование не разрешено.
...
Сквозная проверка тракта — настоящая запись, расшифровка и разделение по голосам, 87 проверок:
